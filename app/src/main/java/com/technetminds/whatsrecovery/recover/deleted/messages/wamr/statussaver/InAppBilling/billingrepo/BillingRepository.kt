...
import com.technetminds.whatsrecovery.recover.deleted.messages.wamr.statussaver.InAppBilling.billingrepo.BillingRepository.GameSku.CONSUMABLE_SKUS
import com.technetminds.whatsrecovery.recover.deleted.messages.wamr.statussaver.InAppBilling.billingrepo.BillingRepository.GameSku.INAPP_SKUS
import com.technetminds.whatsrecovery.recover.deleted.messages.wamr.statussaver.InAppBilling.billingrepo.BillingRepository.GameSku.SUBS_SKUS
import com.technetminds.whatsrecovery.recover.deleted.messages.wamr.statussaver.InAppBilling.billingrepo.localdb.*
...
class BillingRepository private constructor(private val application: Application) :
    ...
    var MyPurchases = MutableLiveData<List<Purchase>>() // by naveed
    ...
    fun launchBillingFlow(activity: Activity, augmentedSkuDetails: AugmentedSkuDetails) =
            launchBillingFlow(activity, SkuDetails(augmentedSkuDetails.originalJson.toString()))// naveed

    fun launchBillingFlow(activity: Activity, skuDetails: SkuDetails) {
        val oldSku: String? = getOldSku(skuDetails.sku)

//        val oldPurchase: Purchase = getPurchaseForSku(MyPurchases.getValue(), oldSku.toString())// by naveed

            val purchaseParams = BillingFlowParams.newBuilder().setSkuDetails(skuDetails)
//                    .setOldSku(oldSku.toString(), oldPurchase.purchaseToken)//  by  skuDetails.sku naveed  ,  do here
                    .build()
    ...
    override fun onPurchasesUpdated(billingResult: BillingResult, purchases: MutableList<Purchase>?) {
        when (billingResult.responseCode) {
            BillingClient.BillingResponseCode.OK -> {
                ...
                MyPurchases.postValue(purchases)// by naveed
    ...
    private object GameSku {
        val GAS = "gas"
        val PREMIUM_CAR = "whatsrecovery_pro"
        val GOLD_MONTHLY = "whatsrecovery_monthly_subscription"
        val GOLD_YEARLY = "whatsrecovery_yearly_subscription"
